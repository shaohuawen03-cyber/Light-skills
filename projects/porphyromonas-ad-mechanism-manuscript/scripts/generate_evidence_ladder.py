...
stages = [
    ("1", "Aggregate computational", "prioritization", "REACHED", "#DFF3E4", "#176B3A"),
    ("2", "Candidate identities", "and row-level scores", "BLOCKED", "#FFF1D6", "#A35C00"),
    ("3", "Translation and oral", "cohort expression", "NOT TESTED", "#F3F4F6", "#5B6472"),
    ("4", "BBB transport and", "cellular toxicity", "NOT TESTED", "#F3F4F6", "#5B6472"),
...
def main() -> None:
    ...
    parts = [
        ...
        '<text x="900" y="116" class="s" font-size="25" text-anchor="middle">Prediction is a prioritization endpoint; it is not proof of identity, exposure, phenotype, mechanism, or disease causality</text>',
    ]
    ...
    parts += [
        ...
        '<text x="900" y="746" class="b" font-size="23" text-anchor="middle">Release sequences and scores → confirm translation/expression → measure BBB transport and toxicity.</text>',
        '<text x="900" y="780" class="b" font-size="23" text-anchor="middle">Test metal-dependent effects with appropriate controls and biological replication.</text>',
    ...
    text(900, 116, "Prediction is a prioritization endpoint; it is not proof of identity, exposure, phenotype, mechanism, or disease causality", 25, "#486581")
    for i, (num, line1, line2, status, fill, accent) in enumerate(stages):
        x = x0 + i * (box_w + gap)
        if i:
            yy = y + box_h / 2
    ...
    text(900, 746, "Release sequences and scores -> confirm translation/expression -> measure BBB transport and toxicity.", 23)
    text(900, 780, "Test metal-dependent effects with appropriate controls and biological replication.", 23)
