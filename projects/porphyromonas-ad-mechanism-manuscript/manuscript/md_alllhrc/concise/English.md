...
Atanasova et al. treated the acetylcholinesterase–β-amyloid (AChE–Aβ) complex as a computational model of pathogenesis: AChE uses its peripheral anionic site (PAS) as a nucleation centre for Aβ; the complex remains stable in molecular dynamics; Aβ occupies PAS and the adjacent 344–361 stretch; contacts are hydrogen bonds, aromatic packing and hydrophobic interactions; the enzyme backbone does not unfold. Inestrosa et al. defined the PAS as the pathological-chaperone locus that makes AChE–Aβ more neurotoxic than Aβ alone. Silman and Sussman described the same PAS as the electrostatic gate of the 20-Å active gorge. The mechanism below is drawn only from the present docking poses and 100-ns all-atom trajectories, placing periodontitis micropeptides on that AChE–Aβ computational pathway.
...
4. Pathological chaperone activity and amyloid co-nucleation.  
   Inestrosa showed that AChE accelerates conversion of soluble Aβ into toxic fibrils through the PAS. Atanasova’s trajectories recast that process as multi-mode peptide residence on PAS/344–361, with hydrogen bonds and contacts holding the interface while the enzyme acts as a scaffold that lowers the nucleation barrier. The three complexes here yield the same computational picture: a persistent polar network, entrance-loop perturbation, and, for YLSLLQR, SASA contraction (Figure 6C) indicating tighter interfacial burial. Periodontal micropeptides can therefore act as heterologous seeds that recast the PAS electrostatic and hydrophobic landscape and co-nucleate endogenous Aβ on the same surface. In this model the AChE–micropeptide–Aβ assembly carries both gorge occlusion and enhanced amyloid toxicity. Detection of *Porphyromonas gingivalis* in AD brains by Dominy et al. supplies the pathological setting in which an oral ligand reaches central PAS.
