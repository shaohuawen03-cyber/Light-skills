...
The parent workflow specifies GROMACS for trajectory generation and analysis. Its executable 100-ns configuration uses the Amber99SB-ILDN force field, which refines side-chain torsion potentials within the Amber ff99SB family. The system was configured in a triclinic periodic box with a 1.0-nm solute-to-boundary distance, TIP3P-compatible water coordinates, neutralizing ions, and 0.15 mol/L NaCl. These are reported as configured parameters because the complete run logs and binary input files were not available for independent confirmation.
