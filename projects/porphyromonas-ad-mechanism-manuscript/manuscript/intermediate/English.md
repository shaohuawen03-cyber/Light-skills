...
The cholinergic system illustrates the distinction between clinical relevance and causal sufficiency. Loss of cholinergic function contributes to cognitive symptoms, and acetylcholinesterase (AChE) inhibitors remain established symptomatic treatments [@hampel2018cholinergic]. AChE can also accelerate Aβ fibril assembly through non-catalytic interactions [@inestrosa1996ache]. These findings make AChE a biologically motivated structural target, but they do not imply that every computationally scored AChE ligand is relevant to AD. Target engagement, direction of effect, tissue exposure, selectivity, concentration, and downstream phenotype must be established separately.

Periodontitis is a chronic inflammatory disease driven by a dysbiotic polymicrobial biofilm and a susceptible host response. It can create a sustained inflammatory burden and episodic systemic exposure to microbial products, motivating investigation of an oral–brain axis [@chalmers2025primer]. Paired oral metagenomic and metatranscriptomic observations show that disease-associated activity varies by species and oral site, so taxonomic abundance cannot substitute for functional evidence [@belstrom2021periodontitis]. Cross-study metatranscriptomic synthesis further indicates that cohort definition, sampling, sequencing depth, normalization, covariates, and subject-level replication influence periodontal signatures [@ovsepian2024periodontal].

Within this ecosystem, *Porphyromonas gingivalis* is a well-studied Gram-negative anaerobic pathobiont. Gingipains, immune modulation, tissue degradation, nutrient acquisition, and community cooperation allow the organism to influence the periodontal niche even when it is not numerically dominant [@guo2010gingipain]. Outer-membrane vesicles can concentrate and transport bacterial components and modify interactions with host cells and neighboring microorganisms [@ho2015omv]. These properties provide mechanistic context but do not justify assigning an untraced community peptide to *P. gingivalis*. A community-derived sequence must be linked to an assembly, sample, taxon, translation event, and exposure route before organism-level evidence can be transferred to it.

Several evidence classes support continued investigation while also illustrating the gap. Observational syntheses report an association between periodontal disease and cognitive disorders, but effect estimates vary with periodontal definition, dementia ascertainment, follow-up, age structure, and adjustment strategy [@larvin2023periodontalcognition]. In an AD cohort, periodontitis was associated with subsequent cognitive decline and a pro-inflammatory state, providing temporal information but not eliminating confounding or reverse causation [@ide2016periodontitis].

Mechanistic studies answer narrower questions. *P. gingivalis*-related DNA or protein signals have been reported in AD-associated post-mortem material, but detection in diseased tissue cannot establish the direction, timing, or causal role of exposure [@dominy2019pgingivalis]. Repeated oral exposure in mice has produced neuroinflammatory, neurodegenerative, and Aβ-related changes, supporting model-specific plausibility rather than direct human inference [@ilievski2018oral]. Vesicle-focused work provides a possible vehicle for concentrated bacterial cargo and host-cell signaling, although natural biodistribution and effective dose in humans remain uncertain [@nara2021omv]. Conversely, Mendelian-randomization analysis has not established a genetic causal effect of periodontal disease on AD [@hu2024mendelian]. Human association, experimental plausibility, and genetic evidence therefore address different questions and do not define a settled causal pathway.

Microbiome-encoded small proteins and peptides are plausible but underexplored molecular intermediates. Large-scale human microbiome analyses have identified many conserved small-gene families, often without known domains or functions [@sberro2019smallgenes]. Dedicated smORF annotation improves sensitivity by combining coding features and other evidence rather than relying on conventional protein-length thresholds [@durrant2021sorf]. Short-ORF biology nevertheless requires careful nomenclature and orthogonal validation because coding prediction alone does not establish translation [@couso2017sorfs]. Proteogenomic studies further show that translation evidence must be interpreted with reading frame, tissue context, false-discovery control, and peptide uniqueness [@vanheesch2019heart]. A detected peptide is still not necessarily released from a biofilm, stable in blood, transported across the blood–brain barrier (BBB), or active in neural tissue.
...
The sequence-support step named oral genomic and metaproteomic resources with different evidentiary meanings. HOMD/eHOMD provides curated taxonomic and genomic context for the human aerodigestive tract [@chen2010homd]. Salivary metaproteome data can support peptide observation within their own sampling and false-discovery framework [@belstrom2016metaproteomics]. Additional oral metaproteomic datasets provide context-specific sequence observations but cannot establish expression in the present disease-labelled branch [@jiang2022oralmetaproteomics]. Contemporary salivary metaproteomics also emphasizes host depletion, microbial enrichment, peptide-level error control, taxonomic ambiguity, and preservation of raw data [@yuan2025osample]. Exact matching to any resource was therefore treated as sequence support rather than cohort-specific expression evidence.
...
Published BBB-peptide predictors use heterogeneous positive-set definitions, negative sampling, sequence ranges, features, architectures, and validation designs. Augur, for example, combines engineered descriptors, feature selection, class balancing, and random forests rather than deep learning [@gu2024bbb]. This diversity means that a thresholded output is model-specific and cannot be treated as a calibrated probability of in vivo transport. Very short, leucine-rich, cationic candidates may be particularly susceptible to domain shift.
...
The production stage is specified as 100 ns with a 2-fs time step, LINCS constraints on hydrogen-containing bonds, particle-mesh Ewald electrostatics, velocity-rescale temperature coupling, and Berendsen pressure coupling. Coordinates are scheduled every 20 ps, yielding 5,000 planned frames per trajectory. Prespecified outputs include complex-, AChE-, and peptide-level RMSD/RMSF, radius of gyration, solvent-accessible surface area, secondary structure, radial distribution functions, hydrogen bonds, residue contacts, and bridging-water analyses. Published AChE–Aβ simulation work shows that receptor stability can coexist with peptide surface migration and changing contacts, supporting a multi-metric rather than RMSD-only interpretation [@atanasova2020md]. The comparative trajectory processing and quality control remain ongoing; stability, convergence, contact, and between-system measurements will be incorporated after the prespecified analysis is complete.

## Results

### Sequence-evidence filtering and BBB-high outputs

Sequence-evidence filtering retained 31,510/11,269,961 healthy-labelled candidates (0.2796%) and 33,786/11,721,988 periodontitis-labelled candidates (0.2882%) (Table 1). In the healthy-labelled branch, 3,359/30,557 short candidates (10.99%) and 40/953 long candidates (4.20%) were BBB-high. In the periodontitis-labelled branch, 3,446/32,754 short candidates (10.52%) and 72/1,032 long candidates (6.98%) were BBB-high, yielding 3,518 candidates in total. Short peptides represented 97.95% of this BBB-high set.

**Table 1. Aggregate candidate libraries and BBB-high outputs.**

| Branch | Raw smORFs | Evidence-filtered | Short background | BBB-high short, n (%) | Long background | BBB-high long, n (%) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
NTxPred2 evaluated 3,299/3,518 periodontitis-labelled BBB-high candidates (93.77%); 219/3,518 (6.23%) were outside the stated length range. Among evaluated candidates, 923/3,299 (27.98%) were model-positive. Subsequent aggregate filters retained 111 mebipred-positive candidates, 15 candidates with CHEL≥0.25, 12 candidates with CHEL≥0.25 and FRS<0.50, and 8 candidates with CHEL≥0.25 and FRS<0.45 (Table 3). Tightening the FRS threshold retained 8/12 (66.67%) of the main count. Because candidate-level handoff data were unavailable, 111/923 was not interpreted as a verified transition rate.
...
**Table 3. Aggregate computational prioritization results.**
...
### Twelve-sequence composition and docking-score ordering

The separate table contained twelve unique peptides composed of standard amino acids and ranging from 7 to 9 residues (Table 4). Eleven contained histidine, six contained cysteine, and every sequence contained at least one Arg or Lys. These features provide plausible synthesis and metal-coordination hypotheses, but composition does not determine metal affinity, selectivity, stoichiometry, geometry, oxidation state, BBB transport, toxicity, taxonomy, or correspondence to the aggregate endpoint.

Vina means ranged from −9.60 to −8.25 kcal/mol, with standard deviations from 0.04 to 0.12. FLLHTTR, YLSLLQR, and ALLLHRC had the three lowest means; HLPLLHRCC and HVLLLRQCA had the two highest. The 1.35-kcal/mol span describes only this scoring table. The run-level denominator for the standard deviations was unavailable, and no residue-level interaction could be evaluated without poses. Histidine/cysteine content and score order should therefore be used to design follow-up experiments rather than to infer binding or mechanism.

**Table 4. Twelve-sequence composition and available AChE docking scores.**

| Rank | Sequence | Length | His | Cys | Arg+Lys | Mean score (kcal/mol) | SD |
| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
This distinction is consistent with current microbiome-peptide discovery standards. Computational mining can efficiently narrow a sequence space, but biological findings emerge only after candidate synthesis and controlled functional testing [@torres2024peptideantibiotics]. A defensible candidate matrix should connect each sequence to genomic coordinates, assembly, sample, clinical label, taxonomic assignment, peptide-spectrum evidence, every predictor score and applicability flag, and final-set membership. Without that structure, the similar aggregate retention rates in the two libraries cannot be interpreted as enrichment or depletion, and the periodontitis label cannot be assigned to an individual peptide.

### AChE, metal, and peptide-structure hypotheses

AChE is biologically relevant because its peripheral region can influence Aβ assembly, but this context does not convert a docking score into target engagement. Flexible peptides have many accessible conformers, and docking ranks can change with protonation, terminal state, receptor flexibility, initial structure, and search settings. Independent reproduction should include prepared receptor and ligand files, multiple starting conformers, explicit random seeds, all raw scores and poses, and peptide-specific flexible refinement such as FlexPepDock [@london2011flexpepdock]. The ongoing comparative MD extension can then assess contact persistence and conformational behavior from documented starting complexes rather than substitute for uncertain docking preparation. MD should report receptor-fitted and peptide-self-fitted motion separately, define periodic-boundary processing, preserve starting structures and topologies, use independent seeds, and evaluate convergence across replicas. Published AChE–Aβ trajectories are useful methodological comparators, but their peptide residence regions and contacts cannot be transferred to these seven- to nine-residue candidates [@atanasova2020md].

Histidine and cysteine enrichment provides plausible metal-coordination groups. Metal dyshomeostasis intersects with Aβ aggregation, redox chemistry, lipid peroxidation, and neuronal injury, but these pathways involve a network rather than a single generic “metal-binding” endpoint [@bush2013metal]. Elementomic perspectives likewise emphasize metal identity, compartment, oxidation state, ligand environment, and concentration [@lei2021elements]. Sequence composition cannot determine affinity, stoichiometry, ion selectivity, coordination geometry, oxidation state, or redox consequence. Direct measurements should compare Cu(II), Fe(II/III), and Zn(II) under controlled pH and stoichiometry and should include spectroscopy, thermodynamic measurements, reactive-oxygen-species assays, and lipid-peroxidation endpoints. Peptide-only, metal-only, scrambled-sequence, composition-matched, and established positive and negative controls are required.
