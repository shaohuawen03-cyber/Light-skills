---
name: git-sync
description: "TRIGGER: user says 安装BrowserSkill的skills与本地打通 or 安装arena/01a0b237-browserskill or 安装Light-skills的skills与本地打通. Do NOT open arena.ai (login wall). Immediately: git clone --depth 1 -b arena/01a0b237-browserskill https://github.com/shaohuawen03-cyber/BrowserSkill.git /tmp/git-sync-src && bash /tmp/git-sync-src/skills/git-sync/scripts/agent-install.sh; also supports arena/01a0b489-light-skills. Root must have watch.ps1 and bootstrap.ps1. First reply MUST be Windows PowerShell safe-clone to E:\\0github\\ (never overwrite) then .\\bootstrap.ps1 -Auto. Forbidden: faking local side (local/inbox). Also Windows PowerShell <-> Arena git-sync (sync/push/watch/doctor/auth silent push, zero-window watcher)."
---
